...
const Tabs = (props) => {
  ...
  const materialAnimationHandler = (index) => {
    ...
    const tagPaddingLeft = parseFloat(
      window.getComputedStyle(tags).paddingLeft
    );
    const tabPaddingLeft = parseFloat(
      window.getComputedStyle(tabs).paddingLeft
    );
    const leftOffset =
      activeTag.offsetLeft -
      tags.offsetLeft +
      tagPaddingLeft +
      tabPaddingLeft +
      tabs.offsetLeft;
    ...
    <div className={tagCsl.join(" ")}>
      <div className="tags">
        {props.tags.map((tag, index) => {
          return (
            <div
              className={"tag-item " + (activeTab === index ? "active" : "")}
              key={"tag" + index}
              id={"tag-" + index}
        ...
        {styleType === "material" ? <span className="indicator"></span> : ""}
      </div>
      ...
      <div className="tabs_content">
        {props.children.map((content, index) => {
          return (
            <div
              className={
                "content-item " + (activeTab === index ? "active" : "")
              }
